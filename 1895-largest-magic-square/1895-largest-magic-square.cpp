class Solution 
{
    ...
    {
        for(int i=1, checker=0; i<s and flag; i++, checker=0)                 // checking if all rows or columns have equal sum
        {
            for(int j=0; j<s; j++)
            ...
            for(int row=0; row<=m-s; row++)
                for(int col=0; col<=n-s; col++)
                {
                    ...
                    if(!check(grid, row, col, s, rs, 0, true, false))       // if not then skipping to next iteration
                        continue;
                    if(!check(grid, row, col, s, cs, 0, true, true))        // if not then skipping to next iteration
                        continue;
                    else 
